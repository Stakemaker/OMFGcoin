...
// OMFGcoin: optional setting to unlock wallet for block minting only;
...
// OMFGcoin: total coins staked (non-spendable until maturity)
       ...
       BOOST_FOREACH(const CWalletTx* pcoin, vCoins)
       {
            ...
            for (unsigned int i = 0; i < pcoin->vout.size(); i++)
            {
                ...
                if (pcoin->nTime > nSpendTime)
                    continue;  // OMFGcoin: timestamp must not exceed spend time
...

    {
        ...
        CTxDB txdb("r");
        {
            ...
            loop
            {
                ...
                // OMFGcoin: sub-cent change is moved to fee
                ...
                if (nChange > 0)
                {
                    ...
                    else
                    {
                        ...
                        if (!GetBoolArg("-avatar")) // OMFGcoin: not avatar mode
                        {
...
// OMFGcoin: create coin stake transaction
...
// OMFGcoin: check 'spent' consistency between wallet and txindex
// OMFGcoin: fix wallet spent state according to txindex
...
// OMFGcoin: disable transaction (only for coinstake)
