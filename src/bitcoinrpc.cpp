...
Value stop(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            "stop\n"
            "Stop OMFGcoin server.");
    // Shutdown will take long enough that the response should get back
    StartShutdown();
    return "OMFGcoin server stopping";
}
...
// OMFGcoin: get network Gh/s estimate
...
Value getnewaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw runtime_error(
            ...
            "Returns a new OMFGcoin address for receiving payments.  "
...
Value getaccountaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "Returns the current OMFGcoin address for receiving payments to this account.");
...
Value setaccount(const Array& params, bool fHelp)
{
    ...
    if (!address.IsValid())
        throw JSONRPCError(-5, "Invalid OMFGcoin address");
...
Value sendmany(const Array& params, bool fHelp)
{
    ...
    BOOST_FOREACH(const Pair& s, sendTo)
    {
        ...
        if (!address.IsValid())
            throw JSONRPCError(-5, string("Invalid OMFGcoin address:")+s.name_);
...
Value walletpassphrase(const Array& params, bool fHelp)
{
    ...
    // OMFGcoin: if user OS account compromised prevent trivial sendmoney commands
    if (params.size() > 2)
...
Value encryptwallet(const Array& params, bool fHelp)
{
    ...
    StartShutdown();
    return "wallet encrypted; OMFGcoin server stopping, restart to run with encrypted wallet";
}
...
Value getwork(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(-9, "OMFGcoin is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(-10, "OMFGcoin is downloading blocks...");
...
// OMFGcoin: get information of sync-checkpoint
...
// OMFGcoin: reserve balance from being staked for network protection
...
// OMFGcoin: check wallet integrity
...
// OMFGcoin: make a public-private key pair
...
Value createmultisig(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 2)
    {
        string msg = "createmultisig nrequired [\"key\",...]\n"
            ...
            "OMFGcoind createmultisig 2 \"[\\\"PCHAhUGKiFKDHKW8Pgw3qrp2vMfhwWjuCo\\\",\\\"PJrhyo8CUvFZQT8j67Expre2PYLhavnHXb\\\"]\""
            "\nAs a json rpc call\n"
...
string HTTPPost(const string& strMsg, const map<string,string>& mapRequestHeaders)
{
    ostringstream s;
    s << "POST / HTTP/1.1\r\n"
      << "User-Agent: OMFGcoin-json-rpc/" << FormatFullVersion() << "\r\n"
      << "Host: 127.0.0.1\r\n"
...
void ThreadRPCServer2(void* parg)
{
    ...
    if (mapArgs["-rpcpassword"] == "")
    {
        ...
        string strWhatAmI = "To use OMFGcoin(OMFGcoind)";
